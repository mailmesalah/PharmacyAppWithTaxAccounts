...
namespace WpfServerApp.Services
{
    public class ProductService : IProduct
    {
            ...
            lock (Synchronizer.@lock)
            {
                ...
                using (var dataB = new Database9004Entities())
                {
                    ...
                    try
                    {
                        ...
                        lr.purchase_interstate_tax = oProductRegister.PurchaseInterstateTax;
                        lr.purchase_wholesale_tax = oProductRegister.PurchaseWholesaleTax;
                        lr.sales_interstate_tax = oProductRegister.SalesInterstateTax;
                        lr.sales_wholesale_tax = oProductRegister.SalesWholesaleTax;
                        lr.sales_local_tax = oProductRegister.SalesLocalTax;
        ...
        public CProductRegister ReadProductRegister(string productCode)
        {
            ...
            using (var dataB = new Database9004Entities())
            {
                ...
                if (cps.Count() > 0)
                {
                    ...
                    ccp.PurchaseInterstateTax = (decimal)cp.purchase_interstate_tax;
                    ccp.PurchaseWholesaleTax = (decimal)cp.purchase_wholesale_tax;
                    ccp.SalesInterstateTax = (decimal)cp.sales_interstate_tax;
                    ccp.SalesWholesaleTax = (decimal)cp.sales_wholesale_tax;
                    ccp.SalesLocalTax = (decimal)cp.sales_local_tax;
        ...
        public List<CProduct> ReadAllProducts()
        {
            ...
            using (var dataB = new Database9004Entities())
            {
                var datas = dataB.product_register.Select(e => new { e.product_code, e.product, e.type,e.stockin_unit_code,e.stockout_unit_code,e.purchase_interstate_tax,e.purchase_wholesale_tax,e.sales_interstate_tax,e.sales_wholesale_tax,e.sales_local_tax }).Where(e => e.type == "BProduct").OrderBy(e => e.product);
                foreach (var item in datas)
                {
                    products.Add(new CProduct() { Product = item.product, ProductCode = item.product_code, ProductType = item.type, StockInUnitCode=item.stockin_unit_code,StockOutUnitCode=item.stockout_unit_code, PurchaseInterstateTax=(decimal)item.purchase_interstate_tax, PurchaseWholesaleTax = (decimal)item.purchase_wholesale_tax, SalesInterstateTax = (decimal)item.sales_interstate_tax, SalesWholesaleTax = (decimal)item.sales_wholesale_tax, SalesLocalTax = (decimal)item.sales_local_tax });
                }
            }
        ...
        public ObservableCollection<CProduct> ReadAllProductsWithGroupAsTree()
        {
            ...
            try
            {
                using (var dataB1 = new Database9004Entities())
                {
                    using (var dataB2 = new Database9004Entities())
                    {
