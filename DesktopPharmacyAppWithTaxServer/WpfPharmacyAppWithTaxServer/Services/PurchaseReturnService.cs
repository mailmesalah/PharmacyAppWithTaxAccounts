...
{   
    ...
    {        
                ...
                using (var dataB = new Database9004Entities())
                {
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < oPurchaseReturn.Details.Count; i++)
                        {
                            ...
                            pt.tax = oPurchaseReturn.Details.ElementAt(i).Tax;
            ...
            using (var dataB = new Database9004Entities())
            {
                ...
                if (cps.Count() > 0)
                {
                    ...
                    foreach (var item in cps)
                    {
                        ...
                        ccp.Details.Add(new CPurchaseReturnDetails() { SerialNo = (int)item.serial_no, ProductCode = item.product_code, Product = item.product, PurchaseReturnUnit = item.purchase_unit, PurchaseReturnUnitCode = item.purchase_unit_code, PurchaseReturnUnitValue = (decimal)item.purchase_unit_value, Quantity = (decimal)item.quantity, PurchaseReturnRate = (decimal)item.purchase_rate, MRP = (decimal)item.mrp, Total = (netValue + taxValue), Barcode = item.barcode, Tax = (decimal)item.tax, ProductDiscount = (decimal)item.product_discount, InterstateRate = (decimal)item.interstate_rate, WholesaleRate = (decimal)item.wholesale_rate, GrossValue = grossValue, NetValue = netValue, TaxValue = taxValue, OldQuantity = (decimal)item.quantity*-1, ExpiryDate=item.expiry_date, Batch=item.batch});
                    }
                }
        ...
        private void CreateAccountTransaction(CPurchaseReturn oPurchaseReturn, string billType)
        {
            ...
            //Finding bill amount
            decimal billAmount = oPurchaseReturn.Details.Sum(e => ((e.Quantity * e.PurchaseReturnRate) - e.ProductDiscount) + (((e.Quantity * e.PurchaseReturnRate) - e.ProductDiscount) * e.Tax / 100)) + oPurchaseReturn.Expense;
        ...
        //Reports
        public List<CPurchaseReturnReportDetailed> FindPurchaseReturnDetailed(DateTime startDate, DateTime endDate, string billType, string billNo, string supplierCode, string supplier, string productCode, string product, decimal tax, string batch, DateTime expiryDate, string narration, string financialCode)
        {
            ...
            using (var dataB = new Database9004Entities())
            {
                ...
                string subQ = billNoQuery + productCodeQuery + productQuery + narrationQuery + financialCodeQuery + billTypeQuery + expiryDateQuery + batchQuery + taxQuery + supplierCodeQuery + supplierQuery;
                
                var resData = dataB.Database.SqlQuery<CPurchaseReturnReportDetailed>("Select  ((bd.quantity * bd.purchase_rate)-bd.product_discount)*-1 As NetValue, (((bd.quantity * bd.purchase_rate)-bd.product_discount) * bd.tax/100)*-1 As TaxValue, (bd.quantity * bd.purchase_rate)*-1 As GrossValue, bd.quantity*-1 As Quantity, (((bd.quantity * bd.purchase_rate)-bd.product_discount)*bd.tax/100)+ ((bd.quantity * bd.purchase_rate) - bd.product_discount)*-1 As Total, bd.expiry_date As ExpiryDate, bd.batch As Batch, bd.mrp As MRP, bd.wholesale_rate As WholesaleRate, bd.interstate_rate As InterstateRate, bd.purchase_rate As PurchaseRate, bd.product_discount As ProductDiscount, bd.tax As Tax, bd.purchase_unit As PurchaseUnit, bd.product As Product, bd.serial_no As SerialNo, bd.extra_charges As Expense, bd.discounts As Discount, bd.advance As Advance, bd.bill_date_time As BillDateTime,bd.bill_no As BillNo, bd.narration As Narration, bd.financial_code As FinancialCode, bd.supplier As Supplier, bd.supplier_address As SupplierAddress From product_transactions bd Where(bd.bill_date_time >= '" + startD + "' && bd.bill_date_time <='" + endD + "') " + subQ + " Order By bd.bill_date_time,bd.bill_no, bd.serial_no");
                ...
                report.Add(new CPurchaseReturnReportDetailed() { BillDateTime = null, SerialNo = null, Product = "Total", Quantity = quantity, GrossValue = grossValue, TaxValue = taxValue, NetValue = netValue, ProductDiscount = proDiscount, Total = total });
        ...
        public List<CPurchaseReturnReportSummary> FindPurchaseReturnSummary(DateTime startDate, DateTime endDate, string billType, string billNo, string supplierCode, string supplier, string productCode, string product, decimal tax, string batch, DateTime expiryDate, string narration, string financialCode)
        {
